    ...
    methods:{
        ...
        sendCookies(){
            ...
            for(let i=0;i<this.Adults.length;i++){
                ...
                if(this.containCheck(this.Adults[i].fname,"number_and_special")) return alert("Firstname must only contains English alphabets. \n ชื่อผู้โดยสารต้องเป็นตัวอักษรภาษาอังกฤษเท่านั้น และไม่มีตัวอักษรพิเศษ")
                if(this.containCheck(this.Adults[i].lname,"number_and_special")) return alert("Lastname must only contains English alphabets. \n นามสกุลผู้โดยสารต้องเป็นตัวอักษรภาษาอังกฤษเท่านั้น และไม่มีตัวอักษรพิเศษ")
            ...
            for(let i=0;i<this.Children.length;i++){
                if(this.Children[i].fname==="" || this.Children[i].lname==="" || this.Children[0].prefix===""){
                    return alert("please fill every field in the form.")
                }

                if(this.containCheck(this.Children[i].fname,"number_and_special")) return alert("Firstname must only contains English alphabets. \n ชื่อผู้โดยสาร(เด็ก)ต้องเป็นตัวอักษรภาษาอังกฤษเท่านั้น และไม่มีตัวอักษรพิเศษ")
                if(this.containCheck(this.Children[i].lname,"number_and_special")) return alert("Lastname must only contains English alphabets. \n นามสกุลผู้โดยสาร(เด็ก)ต้องเป็นตัวอักษรภาษาอังกฤษเท่านั้น และไม่มีตัวอักษรพิเศษ")
        ...
        containCheck(str,check){
            
            if (check === "number") return /[0-9]/.test(str)
            if (check === "special") return /[ -/:-@[-`{-~]/.test(str)
            if (check === "number_and_special") return /[0-9 -/:-@[-`{-~]/.test(str)
            if (check === "email") return /@/.test(str) && /./.test(str)
        }
    },
    created(){
